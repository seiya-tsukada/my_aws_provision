...
BUCKET_NAME="my-provision.whiteray.tokyo"
...
/usr/bin/aws s3 cp s3://${BUCKET_NAME}/git-keys/id_git_rsa ~/.ssh/id_rsa
/usr/bin/aws s3 cp s3://${BUCKET_NAME}/ssh/config ~/.ssh/config
/usr/bin/aws s3 cp s3://${BUCKET_NAME}/provision/provision.yml /tmp/provision.yml
